...
XPCOMUtils.defineLazyModuleGetter(this, "SessionConverter", "resource://sessionmanager/modules/session_convert.jsm");
...
// Session larger than this will be written in multiple write passes to prevent an out of memory error
const SESSION_MAX_WRITE_SIZE = 20971520;  // 20 MB

//
// private variables
//
var _mUserDirectory = null;
...
this.SessionIo = {
	...
	set _mUserDirectory(aDir) {
		_mUserDirectory = aDir;
	},
	...
	closeSession: function(aWindow, aForceSave, aKeepOpen) {
		return Private.closeSession(aWindow, aForceSave, aKeepOpen);
	},
	...
	backupCurrentSession: function(aEnteringPrivateBrowsingMode, aPeriodicBackup) {
		return Private.backupCurrentSession(aEnteringPrivateBrowsingMode, aPeriodicBackup)
	},
...
let Private = {
	...
	save: function(aWindow, aName, aFileName, aGroup, aOneWindow, aValues)
	{
		...
		if ((!aWindow && aOneWindow) || (!aOneWindow && Utils.isPrivateBrowserMode()) || (aOneWindow && Utils.isPrivateWindow(aWindow)) || !Utils.getBrowserWindows().length) 
			return;
		
		// Save Window should be modal
		let values = aValues || { text: aWindow ? (Utils.getFormattedName((aWindow.content.document.title || "about:blank"), new Date()) || (new Date()).toLocaleString()) : "", 
					...
					if (Components.isSuccessCode(aResults)) {
						...
						if (!values.append) {
							...
							if (!aOneWindow)
							{
								...
								else if (SharedData._autosave_filename == file.leafName)
								{
	...
	load: function(aWindow, aFileName, aMode, aSessionState)
	{
		...
		if ((/^window/.test(matchArray[3]) && SharedData.mActiveWindowSessions[aFileName]) ||
		    (/^session/.test(matchArray[3]) && (SharedData._autosave_filename == aFileName)))
		{
		...
		if (!startup && !use_new_window) {
			...
			if (!newWindow && overwriteTabs && !overwrite_window)
			{
				...
				if (SharedData._autosave_filename) 
				{
		...
		if (!aSessionState && !Utils.isPrivateWindow(aWindow) && (overwriteTabs || startup) && !altWindow)
		{
			...
			if (!newWindow && !overwrite_window && SharedData._autosave_filename=="" && (matchArray2 = /^session\/?(\d*)$/.exec(matchArray[3]))) 
			{
				let time = parseInt(matchArray2[1]);
	...
	closeSession: function(aWindow, aForceSave, aKeepOpen)
	{
		log("closeSession: " + ((aWindow) ? aWindow.com.morac.SessionManagerAddon.gSessionManagerWindowObject.__window_session_filename : SharedData._autosave_filename) + 
		    ", aForceSave = " + (aForceSave || SharedData.mAboutToEnterPrivateBrowsing) + ", aKeepOpen = " + (aKeepOpen || SharedData.mAboutToEnterPrivateBrowsing), "DATA");
		let filename = (aWindow) ? aWindow.com.morac.SessionManagerAddon.gSessionManagerWindowObject.__window_session_filename : SharedData._autosave_filename;
		let name = (aWindow) ? aWindow.com.morac.SessionManagerAddon.gSessionManagerWindowObject.__window_session_name : SharedData._autosave_name;
		let group = (aWindow) ? aWindow.com.morac.SessionManagerAddon.gSessionManagerWindowObject.__window_session_group : SharedData._autosave_group;
		let time = (aWindow) ? aWindow.com.morac.SessionManagerAddon.gSessionManagerWindowObject.__window_session_time : SharedData._autosave_time;
		if (filename)
		{
			...
			if (aForceSave || SharedData.mAboutToEnterPrivateBrowsing || !Utils.isPrivateWindow(aWindow)) {
				try
				{
					this.writeFile(file, SessionDataProcessing.getSessionState(name, aWindow, Utils.getNoUndoData(), true, group, null, time, SharedData.mClosingWindowState || (!aWindow && SharedData.mShutdownState)), function(aResults) {
						if (Components.isSuccessCode(aResults)) {
							// Update SQL cache file
							SQLManager.addSessionToSQLCache(false, file.leafName);
						}
			...
			if (!aKeepOpen) {
				...
				else if (!SharedData.mAboutToEnterPrivateBrowsing) {
	...
	saveWindowSession: function(aWindowSessionData, aWindowState) {
		...
		if (aWindowSessionData[0]) {
			...
			try
			{
				...
				this.writeFile(file, SessionDataProcessing.getSessionState(aWindowSessionData[1], true, Utils.getNoUndoData(), true, aWindowSessionData[2], null, aWindowSessionData[3], window_session), function(aResults) {
					if (Components.isSuccessCode(aResults)) {
						// Update SQL cache file
						SQLManager.addSessionToSQLCache(false, file.leafName);
					}
	...
	sanitize: function(aRange)
	{
		...
		if (aRange && (typeof aRange[0] == "number")) {
			...
			for (var i=0; i<2; i++) {
				let sessions = (i==0) ? this.getSessions() : this.getSessions(null, Utils._string("deleted_sessions_folder"));
				let folder = (i==0) ? "" : Utils._string("deleted_sessions_folder");
			...
			if (error) Utils.ioError(ex, errorFileName);
		}
		else {
			try {
	...
	getProfileFile: function(aFileName)
	{
		let file = SharedData.mProfileDirectory.clone();
	...
	getUserDir: function(aFileName)
	{
		...
		try {
			...
			if (_mUserDirectory) dir = _mUserDirectory.clone();
			else dir = null;
		} finally {
			return dir;
		}
	...
	cacheSessions: function(aSubFolder) {
		...
		var callback = {
			timer: null, // Needed to prevent timer from being garbage collected which will stop timer (see Firefox bug 640629 for details)
	...
	autoSaveCurrentSession: function(aForceSave)
	{
		try
		{
			if (aForceSave || !Utils.isPrivateBrowserMode()) {
				...
				this.writeFile(this.getSessionDir(Constants.AUTO_SAVE_SESSION_NAME), state, function(aResults) {
					if (Components.isSuccessCode(aResults)) {
						...
						let exception = new Components.Exception(Constants.AUTO_SAVE_SESSION_NAME, Cr.NS_ERROR_FILE_ACCESS_DENIED, Components.stack.caller);
						Utils.ioError(exception);
					}
				});
			}
	...
	backupCurrentSession: function(aEnteringPrivateBrowsingMode, aPeriodicBackup)
	{
		...
		if (backup == 2)
		{
			...
			if (results == -1) {
				let saveRestore = !(PreferenceManager.get("browser.sessionstore.resume_session_once", false, true) || this.doResumeCurrent() || aEnteringPrivateBrowsingMode);
		...
		if ((backup > 0) || temp_backup) {
			...
			let helper_state = (SharedData.mShutDownInPrivateBrowsingMode || Utils.isPrivateBrowserMode()) ? SharedData.mBackupState : null;
			log("backupCurrentSession: helper_state = " + helper_state, "DATA");
		
			try {
				state = SessionDataProcessing.getSessionState(Utils._string("backup_session"), null, Utils.getNoUndoData(), null, Utils._string("backup_sessions"), true, null, helper_state);
			} catch(ex) {
				logError(ex);
			}
			try {
			...
			try
			{
				this.writeFile(this.getSessionDir(Constants.BACKUP_SESSION_FILENAME), state, function(aResults) {
					if (Components.isSuccessCode(aResults)) {
						...
						let exception = new Components.Exception(Constants.BACKUP_SESSION_FILENAME, Cr.NS_ERROR_FILE_ACCESS_DENIED, Components.stack.caller);
						Utils.ioError(exception);
					}
				});
	...
	asyncReadFile: function(aFile, aCallback)
	{
		...
		let channel = Services.io.newChannelFromURI(fileURI);
	...
	delFile: function(aFile, aSilent, aDeleteOnly)
	{
		if (aFile && aFile.exists())
		{
			try
			{
				...
				if ((aFile.leafName != Constants.SESSION_SQL_FILE) && (aFile.parent.leafName != Utils._string("deleted_sessions_folder")))
				...
				if (aDeleteOnly || (PreferenceManager.get("recycle_time", 7) <= 0)) {
					...
					let folder = Utils._string("deleted_sessions_folder");
	...
	purgeOldDeletedSessions: function() {
		...
		if (time > (_lastCheckedTrashForRemoval + 86400000)) {
			...
			let dir = this.getSessionDir(Utils._string("deleted_sessions_folder"));
