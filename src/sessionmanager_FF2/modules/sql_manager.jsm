...
let Private = {
	...
	buildStatement: function(aCallbackObject, aDataArray, aWipeDataBaseFirst, aSessionFileName) {
		...
		if (aDataArray.length) {
			...
			timer_callback.timer = Components.classes["@mozilla.org/timer;1"].createInstance(Ci.nsITimer);
			timer_callback.timer.initWithCallback(timer_callback, (PreferenceManager.get("encrypt_sessions") ? 100 : 50), Ci.nsITimer.TYPE_REPEATING_SLACK);
		}
		else {
	...
	removeSessionFromSQLCache: function(aSessionFileName, aDoNotNotfiy) {
		...
		log("Deleting " + (aSessionFileName ? JSON.stringify(aSessionFileName) : "all sessions") + " from SQL cache.", "INFO");
		...
		try {
			statement.executeAsync({
				...
				handleCompletion: function(aReason) {
					...
					log("Deleted " + (aSessionFileName ? JSON.stringify(aSessionFileName) : "all sessions") + " from SQL cache.", "INFO");
