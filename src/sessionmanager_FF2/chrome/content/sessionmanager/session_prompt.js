...
var gSessionManagerSessionPrompt = {
	...
	observe: function(aSubject, aTopic, aData)
	{
		...
		case "private-browsing":
			this.checkPrivateBrowsingMode(aData == "enter", true, this.gParams.autoSaveable);
			break;
	...
	onLoad: function() {
		...
		if (this.modal && !SharedData.sessionPromptData.startup) {
		...
		this.checkPrivateBrowsingMode(Utils.isPrivateBrowserMode(), Utils.isPrivateWindow(window.opener), SharedData.sessionPromptData.autoSaveable, true);
	...
	onUnload: function() {
		...
		if (Application.name.toUpperCase() == "FIREFOX") {
	...
	drawWindow: function() {
		...
		this._("leave_window_open").checked = (PreferenceManager.get("leave_prompt_window_open","").split(",").indexOf(this._("actionButton").label) != -1);
	...
	updateWindow: function(aSelectedFileName) {
		...
		if (SharedData._autosave_name)
			this.gBannedFileNames[SharedData._autosave_name] = true;
	...
	isAcceptable: function(aNotAcceptable) 
	{
		...
		if (this.gTextBoxVisible) {
			...
			var backupSessionName = Constants.BACKUP_SESSION_REGEXP.test(input + ".session");
			this.gTextBox.setAttribute("badname", this.gBackupNames[input] || backupSessionName);
			badSessionName = !input || this.gBackupNames[input] || backupSessionName;
		}

		this.gAcceptButton.disabled = this.gExtraButton.disabled = aNotAcceptable ||
			this.gInvalidTime || badSessionName || badGroupName || (this.gParams.autoSaveable && Utils.isPrivateBrowserMode()) ||
	...
	onSelectMenu: function(aEvent) {
		...
		switch(aEvent.explicitOriginalTarget.id) {
			case "save":
				...
				break;
			case "saveWin":
				SessionIo.saveWindow(Utils.getMostRecentWindow("navigator:browser"));
				break;
			case "load": 
				SessionIo.load();
				break;
	...
	{
		let inPrivateBrowsing = Utils.isPrivateBrowserMode();
	...
	checkPrivateBrowsingMode: function(inPrivateBrowsing, inPrivateWindow, aSaving, aJustOpened)
	{
		...
		let saving = (this._("actionButton").label == this._("save").label);
		let saving_window = (this._("actionButton").label == this._("saveWin").label);
