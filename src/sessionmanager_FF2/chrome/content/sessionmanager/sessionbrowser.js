...
var gSessionManagerSessionBrowser = {
	...
	onUnload_proxy: function(aEvent) {
		...
		if (gSessionManagerSessionBrowser.gObserving) {
			Services.obs.removeObserver(gSessionManagerSessionBrowser, "sessionmanager:update-tab-tree");
			Services.obs.removeObserver(gSessionManagerSessionBrowser, "private-browsing");
		}
	...
	observe: function(aSubject, aTopic, aData)
	{
		...
		case "private-browsing":
			if (this.gSaving) {
				if (aData == "enter") {
					this.gIgnoreUpdates = true;  // Ifnore the TabOpens that come in after leaving private browsing
					this.treeView.initialize();
				}
				else {
					//this.gTabTree.view = this.treeView;
					// Listen for session restore and then re-initialize tree
					Services.obs.addObserver(gSessionManagerSessionBrowser, "sessionstore-browser-state-restored", false);
					this.gWaitingForRestore = true;
				}
			}
			break;
	...
	initTreeView: function(aFileName, aDeleting, aStartupPrompt, aSaving, aUpdate) {
		...
		if (aUpdate) {
			...
			this.oneWindow = (gSessionManagerSessionPrompt.gParams.callbackData && gSessionManagerSessionPrompt.gParams.callbackData.oneWindow) ? Utils.getMostRecentWindow("navigator:browser") : null;
		
			// Tab groups only exist in Firefox (only do once per session manager window open)
			if (firstTime && (Application.name.toUpperCase() == "FIREFOX")) {
			...
			if (aSaving && !this.gObserving) {
				...
				Services.obs.addObserver(gSessionManagerSessionBrowser, "private-browsing", false);
			}
			...
			if (aSaving) {
				try {
					// If Private Browsing Mode (Firefox 19 and earlier) don't display any windows
					if (Utils.isPrivateBrowserMode()) 
						state = BLANK_SESSION;
					else
	...
	addWindowStateObjectToTree: function(aWinData, aIx) {
		...
		if (this.aShowWindowSessions) {
			...
			windowSessionName = (windowSessionName) ? (Utils._string("window_session") + "   " + windowSessionName.split("\n")[1]) : null;
		}
	...
	addTabStateObjectToTree: function(aTabData, aWinParentState, aIconURL) {
		...
		var iconURL = aTabData.attributes && aTabData.attributes.image || aIconURL || null;
		...
		// if still no iconURL, try and grab the URL from favicon service.  I don't use this to start with since favicon might
		// not be cached at this time.  Using this will set the default icon if there is none
		if (!iconURL) {
			var faviconService = Cc["@mozilla.org/browser/favicon-service;1"].getService(Ci.nsIFaviconService);
			iconURL = faviconService.getFaviconImageForPage(Services.io.newURI(entry.url,null,null)).spec;
			notweak = true;
		}
	...
	//    - data[4] contains the favicon URL of the tab if data[0] is iconChange.  This is needed because the label isn't always set correctly in SeaMonkey in the window state.
	//
	updateTree: function() {
				...
				if (this.gStateObject.windows[i].extData && (this.gStateObject.windows[i].extData.__SessionManagerWindowId == window_id)) {
					...
					for (var j=0; j<this.gTreeData.length; j++) {
						...
						if (this.gTreeData[j].ix == i) {
							...
							var tabData = (removing || moving) ? null : this.addTabStateObjectToTree(new_window_state.windows[0].tabs[tab_position], loading ? null : this.gTreeData[j], data[4] ? decodeURIComponent(data[4]) : null);
	...
	onTabTreeSelection: function(aEvent) {
		var ix = this.gTabTree.currentIndex;
		if (this.treeView.isContainer(ix) && this.treeView.isContainerOpen(ix)) {
			...
			if (item.tabs) {
				...
				for each (var tab in item.tabs) {
					if ((aType != "TOGGLE") || windowSelected || this.gTabTree.view.selection.isSelected(this.gTreeData.indexOf(tab)))
						tab.checked = (aType == "TOGGLE") ? !tab.checked : (aType == "ALL");
